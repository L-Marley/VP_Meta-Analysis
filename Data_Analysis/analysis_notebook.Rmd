...
Because I'm switching from this multilevel multivariate approach the data are no longer in an appropriate structure. The publication ID for multiple outcome type studies will be made decimal and the study ID dropped.

This is not necessary for the intensity data as no multiple publications per study.

```{r univariate_structure}
# Vividness Data
vivid_dat$Pub_ID[7] <- 379.5
vivid_dat$Pub_ID[11] <- 63.5
vivid_dat <- 
  vivid_dat %>% 
  select(-Study_ID)
```
...
```{r vivid_pmod_for}
vivid_labs <- c("McIsaac & Eich, 2002", 
                "Sutin & Robins, 2010", 
                "Bernsten & Rubin, 2006", 
                "Marcotti & St Jacques, 2017", 
                "Nigro and Neisser, 1983", 
                "Vella and Moulds, 2014, s.1", 
                "Vella and Moulds, 2014, s.2", 
                "Seikiguchi & Nonaka, 2014", 
                "Sutin & Robins, 2010", 
                "Williams and Moulds, 2008, s.1", 
                "Williams and Moulds, 2008, s.2"
                )
forest(vivid_pmod, addfit = T, slab = vivid_labs, cex = .7)
```
...
```{r}
forest(vivid_emo, slab = vivid_labs, addfit = T, cex = .7)
```
